...
int fun8(void){
	...
	if(!sqlite_rd_ifexist(DB_PATH,"stu_info","id",stu_tmp_node->id)){ //不存在
	...
	list_for_each_safe(pos,n,&list_stu_head.list) {  
		//list_del(pos);
		stu_tmp_node2 = list_entry(pos,list_stu_t,list);

		if(!strcmp(stu_tmp_node2->id,stu_tmp_node->id)){
			memcpy(stu_tmp_node2,stu_tmp_node,sizeof(list_stu_t));
		}
	...
	// 6. 将 学生 信息 更新入 数据库 的 stu_info 表
	...
	// 7. 将 学生分数 信息 更新入 数据库 的 score_info 表
